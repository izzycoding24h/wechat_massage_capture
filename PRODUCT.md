...
This product captures authorized, visible WeChat desktop chat windows as ordered screenshot evidence, then writes manifests, metadata, logs, and SHA256 verification files. Success means the user can understand the flow from the home screen, calibrate scrolling on their own computer, run a full capture, stop it reliably with a global hotkey, and locate the output without touching the command line.
...
- Preserve evidence context: every run should make output location and run status obvious.
